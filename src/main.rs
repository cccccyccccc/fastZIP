...
use fastzip::gui::{
    GuiLaunchRequest, create_shell_compression_request, run_native_gui,
    run_shell_compression_progress,
};
use fastzip::localization::{detect_app_locale, localize_message, set_current_locale};
...
#[command(
    name = "fastzip",
    version,
    about = "A native Rust archive tool",
    long_about = None
)]
struct Cli {
    #[command(subcommand)]
...
enum Command {
    ...
    ShellCompress {
        #[arg(long = "format", required = true, value_enum, value_name = "FORMAT")]
        format: ShellCompressionFormatArg,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
enum ShellCompressionFormatArg {
    #[value(name = "7z")]
    SevenZip,
    #[value(name = "zip")]
    Zip,
    ...
    if let Err(error) = run() {
        #[cfg(target_os = "windows")]
        {
            if !has_console_window() {
                set_current_locale(detect_app_locale());
                let description = format!(
                    "{}\n\n{error:#}",
                    localize_message("FastZIP failed to start.", "FastZIP 启动失败。")
                );
...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Command::Gui { archive }) => {
            run_native_gui(archive.map(GuiLaunchRequest::OpenArchive))?;
        }
        Some(Command::ShellCompress {
            format,
            inputs,
            output,
        }) => {
            let request = create_shell_compression_request(
                inputs,
                map_shell_compression_format(format),
                output,
            )?;
            run_shell_compression_progress(request)?;
        }
        Some(Command::Test {
            archive,
            password,
        ...
        Some(Command::Compress {
            ...
            if let Some(ref preset_name) = preset {
                if let Some(preset_value) = fastzip::settings::load_preset(preset_name) {
                    match fastzip::settings::decode_preset_value(&preset_value) {
        ...
        Some(Command::Checksum { files, algo }) => {
            use fastzip::hash::{HashAlgorithm, file_checksum};
        ...
        Some(Command::Benchmark { output }) => {
            use fastzip::benchmark::run_benchmark;
            let out_dir = output.unwrap_or_else(|| PathBuf::from("benchmark_results"));
            let entries = run_benchmark(&out_dir)?;
            println!(
                "{:<12} {:<10} {:<12} {:<14} {:<14} {:<12}",
...
fn map_shell_compression_format(value: ShellCompressionFormatArg) -> CompressionFormat {
    match value {
        ShellCompressionFormatArg::SevenZip => CompressionFormat::SevenZip,
        ShellCompressionFormatArg::Zip => CompressionFormat::Zip,
        ShellCompressionFormatArg::Tar => CompressionFormat::Tar,
        ShellCompressionFormatArg::TarGz => CompressionFormat::TarGz,
        ShellCompressionFormatArg::TarBz2 => CompressionFormat::TarBz2,
        ShellCompressionFormatArg::TarXz => CompressionFormat::TarXz,
        ShellCompressionFormatArg::Gz => CompressionFormat::Gz,
        ShellCompressionFormatArg::Bz2 => CompressionFormat::Bz2,
        ShellCompressionFormatArg::Xz => CompressionFormat::Xz,
        ShellCompressionFormatArg::Zst => CompressionFormat::Zst,
        ShellCompressionFormatArg::TarZst => CompressionFormat::TarZst,
        ShellCompressionFormatArg::Lz4 => CompressionFormat::Lz4,
        ShellCompressionFormatArg::TarLz4 => CompressionFormat::TarLz4,
    }
}
